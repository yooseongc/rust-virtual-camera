...
#define DEFAULT_IMAGE_ROWS 720
#define DEFAULT_IMAGE_COLS 1280
...
namespace winrt::WindowsSample::implementation
{
        ...
        )
    {
        ...
        constexpr FrameSize frameSizes[] = {
            { DEFAULT_IMAGE_COLS, DEFAULT_IMAGE_ROWS },
            { 1920, 1080 },
            { 640, 480 },
        };
        ...
        {
            for (const auto& subtype : { MFVideoFormat_NV12, MFVideoFormat_RGB32 })
            {
                ...
                RETURN_IF_FAILED(MFSetAttributeSize(spMediaType.get(), MF_MT_FRAME_SIZE, frameSize.width, frameSize.height));
                RETURN_IF_FAILED(MFSetAttributeRatio(spMediaType.get(), MF_MT_FRAME_RATE, 30, 1));
                const uint32_t bytesPerFrame = subtype == MFVideoFormat_NV12
                    ? frameSize.width * frameSize.height * 3 / 2
                    : frameSize.width * frameSize.height * 4;
                RETURN_IF_FAILED(spMediaType->SetUINT32(MF_MT_AVG_BITRATE, bytesPerFrame * 8 * 30));
                RETURN_IF_FAILED(MFSetAttributeRatio(spMediaType.get(), MF_MT_PIXEL_ASPECT_RATIO, 1, 1));
                mediaTypeList[mediaTypeIndex++] = spMediaType.detach();
            }
        }
